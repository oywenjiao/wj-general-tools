...
class Redis {
    ...
    /**
     * 在指定的key中对指定的键值做自增或自减操作
     * @param key
     * @param name
     * @param increment
     * @param operate 操作类别：自增(incr), 自减(decr); 默认为incr
     * @returns {*}
     */
    hincrby(key, name, increment = 1, operate = 'incr') {
        if (operate == 'decr') {
            increment = - increment
        }
        return this.client.hincrbyAsync(key, name, increment)
    }
